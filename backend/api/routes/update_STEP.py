...
async def websocket_convert(websocket: WebSocket):
    ...
    try:
        ...
        # STEP to gLTF conversion via OpenCASCADE (pythonocc), same converter as the upload path.
        await websocket.send_json({"status": "wip", "text": "Converting STEP to GLTF..."})
        ...
        try:
            hierarchy, removal_parents = await return_gltf_hierarchy(
                gltf_path,
                graph_name,
                gltf_path,
            )
